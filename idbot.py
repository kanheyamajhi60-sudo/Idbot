...
def admin_wizard_handler(m):
    ...
    logger.info(f"Admin Wizard: cmd={cmd}, step={step}, text='{text}'")
    ...
    # ---- BACK (go to previous step or cancel) ----
    if text == "🍃back":
        if step == 1:
            ...
            # Go back one step
            sess['step'] = step - 1
            # Re‑send the prompt for the previous step
            if cmd == 'link':
                if step == 2:
                    bot.send_message(m.chat.id,
                                     "🔗 *Channel Link Update - Step 1/2*\n\nPlease send the new channel link (invite URL).\nExample: `https://t.me/+abc123`",
                ...
                # For other commands (search, add, block, etc.) they have only one step, so back cancels
                del admin_cmd_sessions[uid]
                bot.send_message(m.chat.id,
                                 f"🥀 Hello, *{m.from_user.first_name}* welcome back to admin👋",
                                 parse_mode='Markdown', reply_markup=admin_main_reply_keyboard())
...
def settings_wizard_handler(m):
    ...
    logger.info(f"Settings Wizard: step={step}, text='{text}'")
    ...
    if text == "👁️ View Full":
        ...
        if val is None:
            ...
            logger.warning(f"View Full: Key {db_key} not found in DB.")
        else:
    ...
    if text == "🍃back":
        if step == 3:
            ...
            # If at step 1, back cancels the wizard
            del settings_sessions[uid]
            bot.send_message(m.chat.id,
                             f"🥀 Hello, *{m.from_user.first_name}* welcome back to admin👋",
                             parse_mode='Markdown', reply_markup=admin_main_reply_keyboard())
        ...
        try:
            if setting == 'link':
                ...
                if link and cid:
                    ...
                    logger.info(f"Settings updated: channel_link={link}, channel_id={cid}")
                else:
                    bot.reply_to(m, "❌ Missing data. Please restart the wizard.")
                    return
            elif setting == 'botapi':
                ...
                if token:
                    ...
                    db.add_log('botapi', f"Updated to: {token}")
                    bot.reply_to(m, "✅ Bot token updated! Bot will restart in 3 seconds...")
                    logger.info(f"Settings updated: bot_token={token}")
                    del settings_sessions[uid]
                    time.sleep(1)
                    restart_bot()
                    return
                ...
                if key:
                    ...
                    logger.info(f"Settings updated: ai_api_key={key}")
                else:
                    bot.reply_to(m, "❌ Missing key.")
                    return
        except Exception as e:
...
def ai_reply_all(m):
    ...
    if db.get_ai_mode(uid):
        ...
        if not key:
            bot.send_message(m.chat.id, "🤖 AI key is missing in database. Please update it via /aiapi.")
            return
